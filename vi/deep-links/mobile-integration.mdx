...
<Tabs>
  <Tab title="iOS (Swift)" icon="apple">
    ...
    <Note>
    **Khuyến nghị.** Thay vì tự viết toàn bộ luồng bên dưới, hãy dùng **Li2 Deep-Link Kit** (`li2-deeplink-kit/ios/`) — một Swift Package "thả vào" (drop-in, không publish lên registry; thêm như local package hoặc copy thẳng `Sources/Li2DeepLink/`). Kit gói sẵn: lời gọi `/track/open`, cổng "một lần mỗi lần cài" (`firstLaunchRan`), cửa sổ chờ 250ms, `UIPasteControl` (đọc clipboard **không** bật alert "Allow Paste"), và việc bóc `li2_cid` khỏi URL đích. Bạn chỉ viết phần **điều hướng**.
    </Note>
