...
<Tabs>
  <Tab title="iOS (Swift)" icon="apple">
    ...
    <Check>Custom domain đã **Verified** + huy hiệu **Deep Links Active** (không phải `*.li2.link`).</Check>
    <Check>File **AASA live**: `curl -s https://your-domain.com/.well-known/apple-app-site-association | jq` trả đúng `<TeamID>.<BundleID>`.</Check>
    <Check>Có **publishable key** dạng `li2_pk_...` (Settings → Analytics → Publishable Key) — **không** phải server API key.</Check>
    <Check>**Associated Domains** trong Xcode khớp đúng domain (`applinks:your-domain.com`).</Check>

    <Note>
    Bỏ qua bước này là nguồn của ~80% sự cố thường gặp (401, "app không mở"). Nếu vướng, xem [Khắc phục sự cố](./troubleshooting).
    </Note>

    #### SDK lo gì — bạn lo gì

    | Li2 SDK gói sẵn | Bạn tự viết |
    |---|---|
    | Gọi `/track/open` (immediate + deferred) | **Điều hướng** theo outcome |
    | Cổng "một lần mỗi lần cài" + cửa sổ chờ 250ms | **Màn hình đồng ý** (UI) |
    | `Li2PasteButton` đọc clipboard không bật alert | Quyết định fallback khi `.missed` / `.failed` |
    | Bóc `li2_cid` khỏi URL đích; cầu nối `Li2.lastClickId` | — |
    ...
    struct ConsentSheet: View {
        ...
        private var primaryButton: some View {
            if #available(iOS 16.0, *), clipboardHasContent != false {
                ...
                // (a) Clipboard rỗng (Li2PasteButton tự disable): phải có nút thay thế —
                // không thì người dùng bị kẹt và outcome `empty` không bao giờ được gửi.
    ...
    **Hai chi tiết không thể bỏ qua:**
    - **(a) Empty-clipboard affordance** — `Li2PasteButton` tự disable khi clipboard rỗng. Không có nút thay thế (`submitPasteControlEmpty()`), người dùng clipboard rỗng bị kẹt và outcome `empty` không bao giờ được gửi.
    ...
    <Accordion title="Hỗ trợ iOS 15 (không có Li2PasteButton)">
    ...
    ```swift
    // Đặt TRƯỚC nhánh else, sau nhánh iOS 16:
    } else if clipboardHasContent != false {
        // iOS 15: đọc trực tiếp — iOS hiện alert "Allow Paste".
    ...
    Sau khi nối xong, chạy thử và **in response của `/track/open`** ra log. Đọc `matchMethod` (`universal_link` / `clipboard` / …) hoặc `missReason` để biết SDK vừa làm gì — đây là công cụ tự chẩn đoán chính. Bảng đầy đủ: [matchMethod & missReason](./track-open-api#giá-trị-matchmethod), cây quyết định theo triệu chứng: [Khắc phục sự cố](./troubleshooting).
    ...
    **Không có zero-grace entry point.** Nếu bạn muốn trì hoãn prompt (ví dụ: sau màn hình onboarding), chỉ cần gọi `requestFirstLaunchConsentAfterGrace()` muộn hơn — các guard nội bộ đảm bảo lời gọi muộn vẫn đúng, và Universal Link đến trong thời gian đó sẽ tự tắt prompt.
    ...
    <Card title="Khắc phục sự cố Deep Link" icon="wrench" href="./troubleshooting">
    ...
    **Universal Link immediate không mang `li2_cid`** — nên gửi nguyên `url.absoluteString` là an toàn, server sẽ phân loại đúng là immediate. `li2_cid` chỉ được Li2 *gieo* trên đường **chưa cài app** (clipboard / install referrer); một cú chạm khi app đã cài được iOS chặn ở chính short link gốc, trước khi có bất kỳ redirect nào sinh `li2_cid`. (Server quyết định immediate vs deferred dựa trên việc `deepLink` **có** `li2_cid` hay không — xem bảng định tuyến trong [API](./track-open-api#server-chọn-đường-xử-lý-như-thế-nào).)
    ...
    <Note>
    **Nhắc lại "Ba định danh".** `li2_cid` là token 16 ký tự định danh **cú click gốc** — Li2 *gieo* nó vào link trên clipboard, bạn chỉ **đọc**. Đừng nhầm với `clickId` trong response (server trả về). Bảng đầy đủ ba định danh: [Tổng Quan → Ba định danh](./overview#ba-định-danh-bạn-sẽ-gặp).
    </Note>
...
<CardGroup cols={2}>
  <Card title="Đo Lường Chuyển Đổi" icon="chart-line" href="./conversion">
