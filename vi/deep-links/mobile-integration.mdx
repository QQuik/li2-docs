...
<Tabs>
  <Tab title="iOS (Swift)" icon="apple">
    ...
    **Không có zero-grace entry point.** Nếu bạn muốn trì hoãn prompt (ví dụ: sau màn hình onboarding), chỉ cần gọi `requestFirstLaunchConsentAfterGrace()` muộn hơn — các guard nội bộ đảm bảo lời gọi muộn vẫn đúng, và Universal Link đến trong thời gian đó sẽ tự tắt prompt.
    ...
    Sau khi deep link đã được phân giải, bạn có thể gắn các **sự kiện kinh doanh thật** — đăng ký, mua hàng — về đúng cú click đã dẫn dắt chúng. Từ `0.2.0`, SDK gói sẵn API này: `trackLead` (3 kiểu), `trackSale` (2 kiểu) và `identify`.

    <Note>
    **Cầu nối tự động.** Một outcome `.matched` lưu lại `Li2.lastClickId` (TTL 30 ngày). Các phương thức **attributed** tự đọc lại click id này, nên một lead/sale gắn về đúng touch-point **mà bạn không phải tự truyền click id** qua code. Đây là toàn bộ lý do module này tồn tại.
    </Note>

    ```
     deep link khớp            lưu lại                     tự đọc
     (resolver .matched)  ──▶  Li2.lastClickId  ──▶  trackAttributedLead / trackAttributedSale
                                (TTL 30 ngày)            (không cần truyền clickId)
    ```
    ...
    **Gọi phương thức nào?** API được **định kiểu theo ý định** — chọn đúng phương thức và quy tắc click id được xử lý sẵn (không còn bẫy `nil` vs `""` như web SDK):
    ...
    **Sale:** `amount` tính bằng **đơn vị nhỏ nhất (cents/xu)** và phải **> 0** — server từ chối 0. `currency` tùy chọn; bỏ trống để dùng mặc định của org.
    ...
    <Steps>
      <Step title="Acquisition tới mua hàng (deferred, ẩn danh)">
        User chạm short link khi **chưa cài** app → cài → mở lần đầu → deep-link match lưu `Li2.lastClickId`. Trước khi đăng nhập, gọi `trackAnonymousLead(eventName: "viewed_pricing")`. Khi user đăng ký, `identify(externalId:)`. Khi họ mua, `trackAttributedSale(externalId:amount:)`. → **Một hồ sơ** mang cả lead lẫn sale, doanh thu quy về đúng chiến dịch.
      </Step>
      <Step title="Re-engagement (immediate, đã định danh)">
        User **đã cài** + đã đăng nhập, chạm short link mở thẳng app (Universal Link). Sau hành động, gọi `trackAttributedLead` / `trackAttributedSale` với `externalId` của họ — clickId từ match tự gắn về.
      </Step>
      <Step title="Sự kiện không từ deep link">
        Mua trong app, không có click nào (mở app trực tiếp). Dùng `trackDirectSale(externalId:amount:)` — `click_id = ""`, không attribution, vẫn ghi nhận doanh thu cho khách hàng.
      </Step>
    </Steps>

    ##### Best practices

    - **Gọi `identify` ngay khi biết danh tính**, trước sale đầu tiên — đó là lằn ranh giữa "một khách hàng" và "hai hồ sơ tách rời".
    - **Dùng `externalId` ổn định** (user id nội bộ của bạn), giống hệt nhau ở lead, `identify` và sale. Lệch một ký tự = tách hồ sơ.
    - **`amount` luôn là số nguyên xu** (`4999` = $49.99). Đừng gửi số thực hay đơn vị lớn; **0 bị từ chối**.
    - **Bỏ trống `currency`** để server dùng mặc định của org — chỉ set khi thật sự cần khác (vd `"vnd"`). Khác với web SDK (cứng `"usd"`).
    - **Đừng tự truyền `clickId`** trừ khi có lý do đặc biệt — để mặc định `Li2.lastClickId` là cách cầu nối hoạt động đúng. Truyền tay dễ gắn nhầm click cũ.
    - **Conversion là best-effort, không hàng đợi/offline buffer**: một POST duy nhất, lỗi thì `throw`. Bắt lỗi và quyết định retry theo nghiệp vụ của bạn (đừng retry mù vô hạn).
    - **Xử lý lỗi rõ ràng**: `.noClickIdAvailable` (chạy match trước hoặc dùng Direct), `.missingExternalId` (thiếu user id ở `identify`), `403` (gói/feature chưa bật), `400` (không thấy khách hàng — cấp `email`/`name`/`phone` để tự tạo).
