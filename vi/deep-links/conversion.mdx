...
<Tabs>
  <Tab title="iOS (Li2 SDK)" icon="apple">
    ...
    <Note>
    Đơn vị nhỏ nhất phụ thuộc loại tiền. Tiền có 2 chữ số thập phân (USD, EUR…) tính bằng **cent/xu**: `4999` = $49.99. Tiền **không có** số thập phân (VND, JPY…) thì `amount` **chính là số đơn vị tiền** — với VND, `amount` là **số đồng trực tiếp** (không nhân /100): `10000` = 10.000đ.
    </Note>
  ...
  <Tab title="Android & HTTP trực tiếp" icon="android">
    ...
    **Payload conversion dùng `snake_case`** (`click_id`, `external_id`, `event_name`…) — **khác** với `/track/open` dùng `camelCase` (`deepLink`, `clipboardStatus`). Đừng bật "convert to snake_case" toàn cục; gửi đúng key như bảng dưới.
    ...
    <Note>
    Đơn vị nhỏ nhất phụ thuộc loại tiền. Tiền có 2 chữ số thập phân (USD, EUR…) tính bằng **cent/xu**: `4999` = $49.99. Tiền **không có** số thập phân (VND, JPY…) thì `amount` **chính là số đơn vị tiền** — với VND, `amount` là **số đồng trực tiếp**: `10000` = 10.000đ.
    </Note>
