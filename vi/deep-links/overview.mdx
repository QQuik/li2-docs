...
Hiện tại Li2 không cung cấp SDK đóng gói riêng. Bạn tích hợp **trực tiếp qua HTTP API** (`POST /api/v1/track/open`), lấy hai ứng dụng mẫu `li2-ios-sample` và `li2-android-sample` làm tham chiếu.
...
<Note>
**IP fallback đang tạm dừng**: Cơ chế đối chiếu deferred theo địa chỉ IP (`LI2_IP_FALLBACK_ENABLED=false`) hiện không hoạt động và là chi tiết nội bộ — bạn không cần tích hợp nó. Hai đường deferred đang chạy thật là **clipboard (iOS)** và **install referrer (Android)**.
</Note>
