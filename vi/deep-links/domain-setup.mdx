...
Li2 **tự động sinh và phục vụ** hai file này ngay khi bạn lưu cấu hình app (xem [Cấu hình trên dashboard](/vi/guide/deep-links/cau-hinh-deep-link-cho-ung-dung-li2)). Nginx proxy các đường dẫn `/.well-known/` công khai về Li2 server, và hostname của request được đọc từ header `X-Forwarded-Host`.
