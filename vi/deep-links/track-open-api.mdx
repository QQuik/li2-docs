...
| Chỉ có `li2Domains` | IP fallback | **Đang tạm dừng** (`LI2_IP_FALLBACK_ENABLED=false`) |
...
```json
{
  ...
  "matchMethod": "universal_link | app_link | clipboard | install_referrer | ip | ip_ua",
...
| `ip` | Fallback theo IP (đang tạm dừng) |
| `ip_ua` | Fallback theo IP + UA (đang tạm dừng) |

## Giá trị `missReason`

| Giá trị | Ý nghĩa |
|---------|---------|
| `no_candidate` | Tra cứu clipboard/IP không có kết quả (hoặc bản ghi đã hết hạn) |
| `ambiguous_candidate` | Tra cứu IP trả về nhiều ứng viên (không kết luận được) |
| `ua_mismatch` | Ứng viên IP có UA hash không khớp |
