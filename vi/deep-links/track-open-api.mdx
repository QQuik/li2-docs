...
- `error_code` là **mã ổn định, máy đọc được** — hãy branch theo nó thay vì so khớp chuỗi `message`. Mọi lỗi phía client (4xx) đều kèm `error_code`; lỗi server (5xx) trả cùng envelope nhưng **không** có `error_code` (chỉ cần retry/backoff).
